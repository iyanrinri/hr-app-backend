...
export class PayrollService {
  ...
  async createPayroll(createPayrollDto: CreatePayrollDto, userId: string): Promise<PayrollDto> {
    const {
      employeeId,
      periodStart,
      periodEnd,
  ...
  async findAll(query: PayrollQueryDto): Promise<PayrollListResponseDto> {
  ...
  async findById(id: string): Promise<PayrollDto> {
  ...
  async markAsPaid(id: string, userId: string): Promise<PayrollDto> {
    const payroll = await this.payrollRepository.findById(id);
    if (!payroll) {
      throw new NotFoundException('Payroll not found');
    }
