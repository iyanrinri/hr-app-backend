...
export class SalaryHistoryController {
  ...
  async create(@Body() createSalaryHistoryDto: CreateSalaryHistoryDto): Promise<SalaryHistoryResponseDto> {
    return this.salaryHistoryService.create(createSalaryHistoryDto);
  }

  @Get()
  @Roles(Role.SUPER, Role.HR, Role.MANAGER)
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Forbidden - SUPER/HR role required' })
  async getByChangeType(@Param('changeType') changeType: string): Promise<SalaryHistoryResponseDto[]> {
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Forbidden - SUPER/HR/MANAGER role required' })
  async findOne(@Param('id', ParseIntPipe) id: number): Promise<SalaryHistoryResponseDto> {
