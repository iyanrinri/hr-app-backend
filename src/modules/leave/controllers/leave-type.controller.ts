...
export class LeaveTypeController {
  ...
  @ApiOperation({ summary: 'Create a new leave type configuration (SUPER/HR only)' })
  @ApiResponse({ 
    status: 201, 
    description: 'Leave type configuration created successfully.',
    type: LeaveTypeConfigResponseDto 
  })
  async create(@Body() createLeaveTypeDto: CreateLeaveTypeConfigDto): Promise<LeaveTypeConfigResponseDto> {
  ...
  async findOne(@Param('id', ParseIntPipe) id: number): Promise<LeaveTypeConfigResponseDto> {
    return this.leaveTypeService.findOne(id);
  }

  @Patch(':id')
  @Roles(Role.SUPER, Role.HR)
  @ApiOperation({ summary: 'Update leave type configuration (SUPER/HR only)' })
  @ApiParam({ name: 'id', type: 'number', description: 'Leave type configuration ID' })
  @ApiResponse({ 
    status: 200, 
  ...
  @Delete(':id')
  @Roles(Role.SUPER, Role.HR)
  @ApiOperation({ summary: 'Delete leave type configuration (SUPER/HR only)' })
  @ApiParam({ name: 'id', type: 'number', description: 'Leave type configuration ID' })
